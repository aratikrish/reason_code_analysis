...
# lt_ct is of type duration as it is the difference of two dates, convert it to numeric for easy plotting
  mutate(x = as.numeric(lt_ct,"days"))
```
...
#TODO calculate how many orders are more than four weeks early; show bin tick marks and labels,
...
pds |> 
  ...
  ggplot(aes(as.numeric(lt_ct,"days") , # lt_ct is duration in days
             fill = otd_reasoncode)) +
...
count_A012_earlier_than_otd_2024q1 <- 
  pds |> 
    filter(revised_agreed_delivery_date_fiscal_quarter_year == "2024-Q1") |> 
    filter(lt_ct < -14,
...
percent_A012_earlier_than_otd_2024q1 <- 
  count_A012_earlier_than_otd_2024q1 / count__2024q1

percent_A012_earlier_than_otd_2024q1
```

chart above for 2023
...
pds |> 
  filter(revised_agreed_delivery_date_fiscal_year == "2023") |> 
  ggplot(aes(as.numeric(lt_ct,"days") , # lt_ct is duration in days
             fill = otd_reasoncode)) +
