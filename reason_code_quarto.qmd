---
title: "GHSC-PSM `r params$taskorder` Global Supply Chain"
...
# Function for exploring character or factor columns -------------------------------------------------------------
...
pds <- pds |>
  filter(
    condom_adjusted_task_order == params$taskorder,
...
percent_reasoncode <- function(input_countrygroup){
numerator <-
  pds |> 
    filter(
      country_group %in% input_countrygroup,
...
percent_tooearly <-  function(input_countrygroup){
  count_A012_earlier_than_5wk <- 
    pds |> 
      filter(
      country_group %in% input_countrygroup,
      revised_agreed_delivery_date_fiscal_quarter_year %in% params$time_period)|>       
  ...
  round((count_A012_earlier_than_5wk / count__all)*100,0)
}
...
```{r dataviz_perfds1_allcountries}
#| output: true
#| title: All countries
...
pds_grouped |> 
  ...
  labs(
    title = str_c("In ", 
                  params$time_period,
                  ", ", 
                  percent_reasoncode(c("All other countries","Congo DRC")),
...
```{r dataviz_perfds1_allcountriesexceptdrc}
#| output: true
#| eval: false
#| title: All countries except DRC
...
pds_grouped |> 
  ...
  labs(
    title = str_c("In ", 
                  params$time_period,
                  ", ", 
                  percent_reasoncode("All other countries"),
                  "% of orders had a reason code applied"),
    subtitle = (""),
    caption = str_c("Data source: GHSC-PSM Artmis\nData last updated:",
                    importdate_pds,
...
```{r dataviz_reasoncode_distribution_nonDRC}
#| eval: false
#| #| output: true
#| title: All countries except DRC
...
pds_allcodes |> 
  filter(
    country_group == "All other countries",
    current_indicator_PO == "A",
    revised_agreed_delivery_date_fiscal_quarter_year %in% params$time_period,
    str_starts(reason_code.y,"AD")) |> 
  ggplot(aes(x = fct_infreq(reason_code.y))) +
...
```{r dataviz_reasoncode_distribution_DRC}
#| eval: false
#| output: true
#| title: DRC
...
```{r dataviz_perfds2_inweeks_allcountries}
#| title: All countries
#| output: true
...
pds |> 
  ...
  labs(
     title = str_c("In ", 
                   params$time_period, 
                   ", ",
                   percent_tooearly(c("All other countries","Congo DRC")),
...
```{r dataviz_perfds2_inweeks_allcountriesexceptdrc}
#| eval: false
#| title: All countries except DRC
#| output: true
...
pds |> 
  filter(country_group == "All other countries",
         revised_agreed_delivery_date_fiscal_quarter_year == params$time_period) |> 
  ggplot(aes(lt_ct/7, fill = otd_reasoncode)) + 
  ...
  labs(
     title = str_c("In ", 
                   params$time_period, 
                   ", ",
                   percent_tooearly("All other countries"),
                   "% of orders were delivered more than 5 weeks ahead of ADD"),
 #   subtitle = (""),
    caption = str_c("Data source: GHSC-PSM Artmis\nData last updated:",
                    importdate_pds,
